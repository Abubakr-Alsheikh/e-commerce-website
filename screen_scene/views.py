...
def index(request):
    ...
    api_url = 'https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key={}&page=1'.format(settings.THEMOVIEDB_API_KEY)
    ...
    if response.status_code == 200:
        ...
        # Extract the movies and featured movie
        movies = data['results']
        featured_movie = movies[0]  # Get the first movie as featured
        ...
        context = {
            ...
            'featured_movie': {
                ...
                'poster_path': featured_movie['poster_path'] 
            },  # Create a dictionary for featured movie with required data
            'IMG_PATH': 'https://image.tmdb.org/t/p/w1280'  # Pass the image path for the template
        }
        ...
        # Handle API errors
        return render(request, 'screen_scene/error.html', {'error': 'Error fetching data from The Movie Database.'})
